...
internal object SentenceContentValidator {
    fun validate(candidates: List<SentenceCandidate>, stored: List<StoredSentence>, errors: MutableList<PackageDiagnostic>, assessments: MutableList<CandidateAssessment>, allowUpdates: Boolean) {
        ...
        CandidateMatcher.assess("sentence", candidates, stored, SentenceCandidate::id, StoredSentence::id,
            ...
            { SentenceContent(it.text, it.translation, it.difficulty, it.vocabularyIds, it.grammarIds) }, { "$.sentences[$it]" }, errors, assessments, allowUpdates)
    }
}
